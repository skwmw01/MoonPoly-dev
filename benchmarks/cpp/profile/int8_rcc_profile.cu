...
namespace moonpoly::int8::row_col_col {
...
bool benchmark_online_splitk(int split_k, int m, int n, int k, int warmup,
                             int iters, int8_t const* a, int8_t const* b,
                             int32_t* c, double& out_ms) {
  setenv("MOONPOLY_INT8_PATTERN3_FORCE_SPLITK",
         std::to_string(split_k).c_str(), 1);
  int alpha = 1;
  int beta = 0;
  try {
    for (int i = 0; i < warmup; ++i) {
      moonpoly::run_int8_gemm(m, n, k, a, false, b, true, c, true, alpha, beta);
    }
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--csv") {
      ...
      std::cout
          << "Usage: moonpoly_profile_int8_rcc [m n k] [options]\n"
  ...
  if (!splitk_csv_path.empty()) {
    ...
    std::cout << "[int8 splitk profile] shapes=" << shapes.size()
    ...
    for (Shape shape : shapes) {
      ...
      for (int split_k : split_ks) {
        ...
        double measured_ms = std::numeric_limits<double>::quiet_NaN();
        double pred_cost =
            moonpoly::int8::row_col_col::gemm_splitk_cost_model(
    ...
    std::cout << "[int8 splitk profile] wrote CSV: " << splitk_csv_path
              << std::endl;
    return 0;
  }
  ...
  std::cout << "[int8 cost-model profile] shapes=" << shapes.size()
  ...
  for (size_t si = 0; si < shapes.size(); ++si) {
    ...
    std::unique_ptr<cutlass::HostTensor<ElementInputB, LayoutInputB>> tensor_b;
    std::unique_ptr<cutlass::HostTensor<ElementOutput, LayoutOutput>> tensor_c;
    int8_t const* a_ptr = nullptr;
    int8_t const* b_ptr = nullptr;
    int32_t* c_ptr = nullptr;
    if (!no_measure) {
      cutlass::gemm::GemmCoord problem_size(shape.m, shape.n, shape.k);
    ...
    int predicted_pid = moonpoly::int8::row_col_col::cutlass_gemm_predict(
    ...
    for (int pid : eval_pids) {
      ...
      if (alignment_ok) {
        pred_cost = moonpoly::int8::row_col_col::gemm_cost_model(
  ...
  std::cout << "[int8 cost-model profile] wrote CSV: " << csv_path << std::endl;
  return 0;
}
