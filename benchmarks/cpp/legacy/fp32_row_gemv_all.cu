...
int run(cublasHandle_t handle, int length_m, int length_n, int length_k,
  ...
  moonpoly::run_moonpoly_gemv(length_m, length_k, tensor_a,
  ...
  cublasStatus_t cu_status = cublasGemmEx(
      ...
      B, CUDA_R_16F, lda, A, CUDA_R_16F, ldb, &fbeta, C, CUDA_R_16F, ldc,
  ...
  // test moonpoly time
  double tmp_time = 1000.0;
  int best_id;
  for(int k_id = 0; k_id<35; k_id++) {
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
    total_time = 0.0;
    milliseconds = 0.0;
    for (int i = 0; i < num_iter; ++i) {
      cudaEventRecord(start);
      moonpoly::run_moonpoly_selected_gemv(k_id, length_m, length_k, tensor_a,
                                         tensor_a.device_data(), tensor_b.device_data(),
                                         tensor_d.device_data(), falpha, fbeta);
      cudaEventRecord(stop);
      cudaEventSynchronize(stop);
    ...
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
    std::cout << "kid:" << k_id << ", best id:" << best_id << ", ct_time: " << ct_time;
    if (ct_time < tmp_time) {
      tmp_time = ct_time;
      best_id = k_id;
    }
    std::cout << ", best_time: " << tmp_time << std::endl;
  }
  return 0;
}
...
int main() {
  int num_case = 3;
  int m_arr[] = {5120, 5120, 5120};
  int k_arr[] = {4096, 2560, 6912};
