...
int run(cublasHandle_t handle, int length_m, int length_n, int length_k,
  ...
  moonpoly::run_moonpoly_selected_gemm_fp16_row_col_col(0, length_m, length_n, length_k,
  ...
  for(int k_id = 0; k_id<40; k_id++) {
    ...
    for (int i = 0; i < num_iter; ++i) {
      cudaEventRecord(start);
      moonpoly::run_moonpoly_selected_gemm_fp16_row_col_col(k_id, length_m, length_n, length_k,
                                                            tensor_a.device_data(), tensor_b.device_data(),
                                                            tensor_d.device_data(), falpha, fbeta);
      cudaEventRecord(stop);
      cudaEventSynchronize(stop);
